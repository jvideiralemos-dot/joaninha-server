...
function sendWhats(to, body, mediaUrl) {
  if (!client || !FROM) {
    console.log("[dev] resposta:", { to, body, mediaUrl });
...
router.post("/", async (req, res) => {
  ...
  if (intent === "cancelar_nota") {
    await sendWhats(from, msg.askClientOrNumberForCancel());
